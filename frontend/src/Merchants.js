...
export default function Merchants() {
  ...
  const handleDelete = id => {
    fetch(`${API}/merchants/${id}`, { method: 'DELETE' })
      .then(() => setMerchants(merchants.filter(m => m.id !== id)));
  };

  const startEdit = merchant => {
  ...
  const handleEdit = e => {
    ...
    fetch(`${API}/merchants/${editId}`, {
      ...
      .then(res => res.json())
      .then(updated => {
        setMerchants(merchants.map(m => (m.id === editId ? updated : m)));
        setEditId(null);
        setEditForm({});
      });
  };
  ...
  return (
    <div style={styles.outer}>
      <div style={styles.container}>
        <div style={styles.title}>Merchants</div>
        ...
        {loading ? (
          ...
          <div style={styles.empty}>No merchants found.</div>
        ) : (
          <table style={styles.table}>
            ...
            <tbody>
              {merchants.map(merchant => (
                editId === merchant.id ? (
                  <tr key={merchant.id}>
                    <td style={styles.td}>{merchant.id}</td>
                    <td style={styles.td}>
                      <input
                        style={styles.input}
                        type="text"
                  ...
                  <tr key={merchant.id}>
                    <td style={styles.td}>{merchant.id}</td>
                    <td style={styles.td}>{merchant.name}</td>
                    <td style={styles.td}>
                      <button style={styles.actionBtn} onClick={() => startEdit(merchant)}>Edit</button>
                      <button style={styles.actionBtn} onClick={() => handleDelete(merchant.id)}>Delete</button>
                    </td>
                  </tr>
                )
              ))}
